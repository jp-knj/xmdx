...
interface SharedTransformContext {
  code: string;
  source: string;
  filename: string;
  ...
  config: {
    expressiveCode: { component: string; moduleId: string } | null;
    expressiveCodeCanRewrite?: boolean;
    starlightComponents: boolean;
    shiki: ((code: string, lang?: string) => Promise<string>) | null;
  };
}

export interface TransformIndexModule {
  transformExpressiveCode(ctx: SharedTransformContext): SharedTransformContext;
  transformShikiHighlight(ctx: SharedTransformContext): Promise<SharedTransformContext>;
}

function createContext(
  overrides: Partial<SharedTransformContext> = {}
...
export function runTransformIndexTests({
  transformExpressiveCode,
  transformShikiHighlight,
}: TransformIndexModule): void {
  ...
  describe('transformShikiHighlight JS string fallback', () => {
    test('highlights JS string code blocks in fallback mode', async () => {
      ...
      const jsStringCodeBlock =
        '"<pre class=\\"astro-code github-dark\\" style=\\"...\\" tabindex=\\"0\\"><code><span>const x = 1;</span></code></pre>"';
      const ctx = createContext({
