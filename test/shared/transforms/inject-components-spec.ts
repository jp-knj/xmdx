...
export interface InjectComponentsModule {
  injectComponentImports(code: string, components: string[], moduleId: string): string;
  injectStarlightComponents(
    code: string,
    config: boolean | { components?: string[]; module?: string }
  ): string;
  injectAstroComponents(code: string): string;
  injectComponentImportsFromRegistry(code: string, registry: Registry): string;
}

export interface InjectComponentsDeps {
  registry: Registry;
  astroComponentsModule: string;
}

export function runInjectComponentsTests({
...
}: InjectComponentsModule & InjectComponentsDeps): void {
...
export default function Content() {
  return <Code lang="js">const x = 1;</Code>;
      ...
      expect(result).toContain(astroComponentsModule);
    });
...
export default function Content() {
  return <Icon name="star" />;
    ...
    it('should inject multiple registry components from multiple modules', () => {
      const code = `
export default function Content() {
  return (
    <>
      ...
      expect(result).toContain("import { Aside, Badge } from '@astrojs/starlight/components';");
      expect(result).toContain("import { Code } from 'astro/components';");
    });

    it('should not inject components only mentioned in strings', () => {
      const code = `
export default function Content() {
  return <div>{"Aside Tabs"}</div>;
}`;
